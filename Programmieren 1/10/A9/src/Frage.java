...
class Pruefung {
	...
	public Frage frageErzeugen(int nummer) {
		...
		int typ = IO.readInt(
				"Fragetyp: Wahr/Falsch (1), Multiple Choice (2), Mehrfachauswahl (3), Lückenfrage (4), Zahlenfolge(5), Ordnungsfrage(6)?");
		switch (typ) {
		default:
		case 1:
		...
		case 3:
			return erzeugeMehrfachauswahlFrage(nummer);
		case 4:
	...
	private Frage erzeugeOrdnungsFrage(int nummer) {
		...
		String[] objekte = new String[anzahl];
		for (int i = 0; i < objekte.length; i++) {
			objekte[i] = IO.readString("Antwort " + i);
		}
		int[] indices = new int[anzahl];
		for (int i = 0; i < indices.length; i++) {
			indices[i] = IO.readInt(i + ". Index:");
		}
		int punkte = IO.readInt("Erreichbare Punkte:");
		return new Ordnungsfrage(text, punkte, objekte, indices);
	}

	private Frage erzeugeZahlenfolge(int nummer) {
		IO.println("Frage " + (nummer) + "(Zahlenfolgen ergaenzen)");
		int anzahl = IO.readInt("Anzahl an vorgegebenen Zahlen:");
		int[] zahlen = new int[anzahl];
		for (int i = 0; i < zahlen.length; i++) {
			zahlen[i] = IO.readInt("Zahl " + i + ":");
		}
		int antwort = IO.readInt("Korrekte Folgezahl:");
		int punkte = IO.readInt("Erreichbare Punkte: ");
		return new Zahlenfolgenerweiterung(zahlen, antwort, punkte);
	}

	private Frage erzeugeLueckenFrage(int nummer) {
		IO.println("Frage " + (nummer) + ":");
		String textVorLuecke = IO.readString("Text vor Lücke: ");
		String luecke = IO.readString("Lücke: ");
		String textNachLuecke = IO.readString("Text nach Lücke: ");
		int punkte = IO.readInt("Erreichbare Punkte: ");
		return new Lueckenfrage(textVorLuecke, luecke, textNachLuecke, punkte);
	}

	private Frage erzeugeMehrfachauswahlFrage(int nummer) {
		...
		int richtigAnzahl = IO.readInt("Anzahl an richtigen Antworten: ");
		int[] richtigIndex = new int[richtigAnzahl];
		for (int i = 0; i < richtigAnzahl; i++) {
			richtigIndex[i] = IO.readInt("Antwortmoeglichkeit " + i + ": ");
		}
		int punkte = IO.readInt("Erreichbare Punkte: ");
		return new Mehrfachauswahl(text, punkte, antworten, richtigIndex);
	}
